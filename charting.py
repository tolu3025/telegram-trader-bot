...
def generate_live_chart(symbol: str, entry: float = None, sl: float = None, tp: float = None, direction: str = None) -> bytes:
    ...
    if entry is not None:
        ax.axhline(y=entry, color='#eab308', linestyle='--', linewidth=1.5, label=f"Entry: {entry:.5f}")
        
    if sl is not None:
        ax.axhline(y=sl, color='#ef4444', linestyle=':', linewidth=1.8, label=f"Stop Loss: {sl:.5f}")
        
    if tp is not None:
        ax.axhline(y=tp, color='#22c55e', linestyle='-.', linewidth=1.8, label=f"Take Profit: {tp:.5f}")
    ...
    title_text = f"📊 {display_name} Live Chart  |  Spot: {current_price:.5f}"
