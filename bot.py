...
async def journal_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ai_feedback = ai_engine.review_journal(notes, trades_summary)
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        analysis = ai_engine.analyze_chart(photo_bytes, caption)
...
async def run_session_analysis(session_key: str, chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    session_mapping = {
        "tokyo": ("Tokyo Session Open", ["AUDUSD=X", "USDJPY=X", "NZDUSD=X"]),
        "london": ("London Session Open", ["EURUSD=X", "GBPUSD=X", "EURGBP=X", "EURJPY=X"]),
        "new_york": ("New York Session Open", ["EURUSD=X", "USDJPY=X", "GC=F", "GBPUSD=X"]),
        "close": ("NY Session Recap", ["EURUSD=X", "GBPUSD=X", "USDJPY=X"])
    }
    ...
    session_name, symbols = session_mapping[session_key]
